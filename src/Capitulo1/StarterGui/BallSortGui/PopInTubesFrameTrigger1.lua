...
local event = ReplicatedStorage:WaitForChild("AbrirTubesFrameEvent")

local frame = script.Parent

local RunService = game:GetService("RunService")

-- Garante que o frame começa invisível
frame.Visible = false
...
event.OnClientEvent:Connect(function()
    -- Antes de mostrar, garante que está invisível e pequeno
    frame.Visible = false
    popInAll(frame, 0.5)
    frame.Visible = true
    task.wait(0.05)
    print("PopInTubesFrameTrigger: Iniciando efeito pop in")
    -- O efeito já está rodando, não precisa chamar popInAll novamente
end)
